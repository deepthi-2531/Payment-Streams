...
| Custodial escrow flow | **Live (hardening in progress)** | Escrow party + proxy-submitted deposit + streamer + refund move real CC (`packages/proxy/src/escrow.ts`). In place: on-chain deposit verification (the deposit is bound to a real payer→escrow CC transfer and the funded total is taken from the verified amount), payer-only + cadence-gated release, validated inputs, atomic store, a pre-release solvency interlock (a release or refund is refused unless the custody party still holds at least the sum of every active escrow's remaining, so one escrow can't spend another's deposit), an aggregate custody cap that bounds the total obligation the single key is trusted with, a cross-process single-writer lock over every store mutation (so a replica or redeploy overlap can't race a double release), and a continuous custody-vs-owed drift monitor. The model is disclosed as operator-custodial. Literal per-escrow isolation (separate parties or an on-ledger lock) and HSM / threshold key custody are the stronger future forms; the first needs infra / DSO settlement rights not available on this network. |
