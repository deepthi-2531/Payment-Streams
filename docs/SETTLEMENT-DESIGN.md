...
| Mandate — trust-minimized pull | **Blocked** | The in-choice `PullCycle` that executes the transfer needs a consistent token-standard DAR set; descoped until the transfer-instruction dep hashes align. Until then the operator can pull only for parties it hosts — and that operator-authority submission of a co-hosted party's funds is now refused unless the deployment sets `ESCROW_DISCLOSED_CUSTODY`, so the "non-custodial" lanes cannot silently become custodial. In the intended topology (payer on its own participant) the property is enforced by Canton; this lane is then custody-safe but feature-incomplete until the on-ledger pull is wired. |
| Custodial escrow flow | **Live (hardening in progress)** | Escrow party + proxy-submitted deposit + streamer + refund move real CC (`packages/proxy/src/escrow.ts`). In place: on-chain deposit verification (a deposit is recorded only when the committed update actually created the funded CC as a holding owned by the escrow party — a delivery, not merely a transfer authorization — and the funded total is the delivered amount, so a locked, still-withdrawable offer can't fund an escrow), payer-only + cadence-gated release with a deterministic per-cycle command id (a crash can't re-pay on restart), validated inputs, atomic store, a pre-release solvency interlock (a release or refund is refused unless the custody party still holds at least the sum of every active escrow's remaining, so one escrow can't spend another's deposit), an aggregate custody cap that bounds the total obligation the single key is trusted with, a cross-process single-writer lock over every store mutation (so a replica or redeploy overlap can't race a double release), a completion rule that keeps an escrow open while a release is an undelivered pending offer and reopens it if that offer expires (so the payer's refund can still recover), and a continuous custody-vs-owed drift monitor. The model is disclosed as operator-custodial. Literal per-escrow isolation (separate parties or an on-ledger lock) and HSM / threshold key custody are the stronger future forms; the first needs infra / DSO settlement rights not available on this network. |
