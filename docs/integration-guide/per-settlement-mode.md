...
**How it works:** The dApp builds a V2 `AllocationRequest` against the asset's V2 interface. The recipient's wallet accepts; this atomically locks the sender's funding against the escrow operator via `AllocationFactory_Allocate(committed=True)`. Per accrual interval, `Allocation_Settle` settles the next leg.
