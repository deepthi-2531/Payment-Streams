 ...
 *   • happy-path Linear / UtilityHoldingCustody
 *   • rejects TokenStandardCustody (STR-103 gate)
...
describe('createStreamSchema', () => {
  it('accepts a valid Linear / UtilityHoldingCustody payload', () => {
    const r = createStreamSchema.safeParse(validBase);
    expect(r.success).toBe(true);
  });

  it('rejects TokenStandardCustody with the STR-103 explanation', () => {
    const r = createStreamSchema.safeParse({
      ...validBase,
      settlementMode: SettlementMode.TokenStandardCustody,
      fundingReference: 'fund-ref',
    });
    expect(r.success).toBe(false);
    if (!r.success) {
      const msg = r.error.issues.map((i) => i.message).join(' ');
      expect(msg).toMatch(/TokenStandardCustody/);
      expect(msg).toMatch(/STR-86/);
    }
  });
