 ...
 * @module featured-app
 *
 * SDK surface for CIP-0047 FeaturedAppActivity integration.
 *
...
/**
 * Estimate the per-tx featured-app reward based on amount + activity
 * type. Per CIP-0047, rewards are capped at ~$1.50 per transaction;
 * actual allocation depends on the validator rewards pool size and
 * total featured-app activity that period.
 *
 * Returns a Decimal estimate in USD-equivalent. Used by dashboards to
 * surface "expected rewards this month" projections; not authoritative.
 */
export function estimateRewardCapUsd(_activityType: ActivityType): Decimal {
